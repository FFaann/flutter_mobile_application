...
enum StopWatchExecute { start, stop, reset, lap ,relax }
...
class StopWatchTimer {
  StopWatchTimer({
    ...
    this.onChangeRawMinute, bool milliSecond,
  }) {
  ...
  final BehaviorSubject<List<StopWatchRecord>> _recordingController =
      BehaviorSubject<List<StopWatchRecord>>.seeded([]);
  ValueStream<List<StopWatchRecord>> get recording => _recordingController;
  ...
  void _relax() {
    ...
    if (_timer != null) {
      ...
      _recording=StopWatchRecord(
        rawValue: rawValue,
        hours: getRawHours(rawValue),
        minute: getRawMinute(rawValue),
        second: getRawSecond(rawValue),
        displayTime: getDisplayTime(rawValue,milliSecond: false, hours: isLapHours),
      );
      print(_recording);
